            ...
            def organization_complete(self, success, output):
                ...
                if success:
                    ...
                    QMessageBox.warning(
                        ...
                        f"The organization process failed:\n\n{output}"
                    )
...
class FixedLayoutMainWindow:
    ...
    # Essential functionality implementations
    def save_current_tokens(self):
        ...
        if token:
            tokens = {'p-b': token}
            if lat_token:
                tokens['p-lat'] = lat_token

            if save_poe_tokens(tokens):
                self.loaded_tokens = tokens
                self.update_token_status_display()
                self.update_config_token_display()
                self.update_config_lat_token_display()
                self.QMessageBox.information(self.window, "Success", "✅ Tokens saved successfully!")
            else:
                self.QMessageBox.warning(self.window, "Error", "❌ Failed to save tokens")
        else:
            self.QMessageBox.warning(self.window, "Warning", "Please enter a token first")

    def on_conversation_selected(self, item):
        """Handle conversation selection and display messages."""
        ...
        self.analyze_conv_btn.setEnabled(True)
        ...
        if messages:
            ...
            self.message_viewer.setHtml("""
                <div style='text-align: center; padding: 20px; color: #888;'>
                    ...
                    <p>Messages will be loaded when you click on a conversation.</p>
                    <p>For now, this shows the conversation metadata.</p>
                </div>
            """)

    def display_conversation_messages(self, messages):
        """Display conversation messages in the viewer."""
        if not messages:
            self.message_viewer.setHtml("<p>No messages to display</p>")
            return
    ...
    def open_conversation_in_browser(self):
        ...
        if url and url != 'No direct URL':
            import webbrowser
            webbrowser.open(url)
            self.status_bar.showMessage(f"🌐 Opened conversation in browser: {url}")
        else:
            self.QMessageBox.warning(self.window, "No URL", "This conversation doesn't have a direct URL")
    ...
    def sort_conversations(self):
        ...
        try:
            if "Date (Newest)" in sort_option:
                self.conversations.sort(key=lambda x: x.get('extracted_at', ''), reverse=True)
            elif "Date (Oldest)" in sort_option:
                self.conversations.sort(key=lambda x: x.get('extracted_at', ''))
            elif "Title (A-Z)" in sort_option:
                self.conversations.sort(key=lambda x: x.get('title', '').lower())
            elif "Messages (Most)" in sort_option:
                self.conversations.sort(key=lambda x: len(x.get('messages', [])), reverse=True)
            elif "Category (A-Z)" in sort_option:
                self.conversations.sort(key=lambda x: x.get('category', 'General').lower())

            # Refresh the display
            self.populate_conversation_list()
            self.status_bar.showMessage(f"✅ Sorted by {sort_option}")

        except Exception as e:
            self.QMessageBox.warning(self.window, "Sort Error", f"Failed to sort: {str(e)}")

    def export_current_conversation(self):
        """Export current conversation to file."""
        if not self.current_conversation:
            self.QMessageBox.warning(self.window, "No Selection", "Please select a conversation first")
            return
            ...
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    if filename.endswith('.json'):
                        ...
                        f.write(f"Extracted: {self.current_conversation.get('extracted_at', 'N/A')}\n\n")

                        messages = self.current_conversation.get('messages', [])
                        for msg in messages:
                            f.write(f"{msg.get('role', 'unknown').upper()}: {msg.get('content', '')}\n\n")

                self.QMessageBox.information(self.window, "Success", f"✅ Exported to {filename}")
                self.status_bar.showMessage(f"✅ Exported conversation to {filename}")

            except Exception as e:
                self.QMessageBox.critical(self.window, "Export Error", f"Failed to export: {str(e)}")
    ...
    def copy_conversation(self):
        ...
        try:
            ...
            self.QMessageBox.warning(self.window, "Copy Error", f"Failed to copy: {str(e)}")
